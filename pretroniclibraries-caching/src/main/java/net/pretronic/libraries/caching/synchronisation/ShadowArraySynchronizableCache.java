...
public class ShadowArraySynchronizableCache<O,I> extends ArraySynchronizableCache<O,I> {

    private final WeakReferenceList<O> shadowReferenceObjects;

    public ShadowArraySynchronizableCache() {
        shadowReferenceObjects = new WeakReferenceList<>();
    }

    public ShadowArraySynchronizableCache(int maxSize) {
        super(maxSize);
        shadowReferenceObjects = new WeakReferenceList<>();
    }

    public ShadowArraySynchronizableCache(int maxSize, int buffer) {
        super(maxSize, buffer);
        shadowReferenceObjects = new WeakReferenceList<>();
    }

    public ShadowArraySynchronizableCache(ExecutorService executor) {
        super(executor);
        shadowReferenceObjects = new WeakReferenceList<>();
    }

    public ShadowArraySynchronizableCache(ExecutorService executor, int maxSize) {
        super(executor, maxSize);
        shadowReferenceObjects = new WeakReferenceList<>();
    }

    public ShadowArraySynchronizableCache(ExecutorService executor, int maxSize, int buffer) {
